...
// --- Data Model ---
        ...
        let lastEditedField = 'recipient';  // 'recipient' or 'send' — which box the user last typed into
        ...
        function setSendCurrency(cur) {
            sendCurrency = cur;
            document.querySelectorAll('#send-currency-toggle .toggle-btn').forEach(b => {
        ...
        function toggleSyriaFields() {
            ...
            const currencyContainer = document.getElementById('syria-currency-container');
            const cityContainer = document.getElementById('syria-city-container');
            const rateContainer = document.getElementById('rate-container');
            const receiveFlag = document.getElementById('receive-currency-flag');
            if (receiveFlag) {
                const receiveCurrency = document.getElementById('syria-currency-input').value;
                receiveFlag.innerHTML = flagImage(receiveCurrency === 'SYP' ? 'sy' : 'us', receiveCurrency + ' flag');
            }

            if (country === 'Syria') {
                currencyContainer.style.display = 'block';
                const currency = document.getElementById('syria-currency-input').value;
                if (currency === 'SYP') {
                    cityContainer.style.display = 'block';
                    rateContainer.style.display = 'none';
                } else {
                    cityContainer.style.display = 'none';
                    rateContainer.style.display = (sendCurrency === 'CAD') ? 'block' : 'none';
                }
            } else {
        ...
        function calculate() {
            ...
            const syriaCurrency = document.getElementById('syria-currency-container').style.display !== 'none' ? document.getElementById('syria-currency-input').value : null;
            ...
            if (isSyriaSYP) {
                amountLabel.innerText = "Recipient Gets (SYP)";
            } else {
                amountLabel.innerText = isJordan ? "Recipient Gets (JOD)" : "Recipient Gets (USD)";
            }
        ...
        document.getElementById('rate-input').addEventListener('input', calculate);
        ...
        async function getUsdCadRate() {
            const target = `https://www.google.com/finance/quote/CAD-USD?hl=en&gl=CA&_=${Date.now()}`;
            const sources = [
                `https://r.jina.ai/http://www.google.com/finance/quote/CAD-USD?hl=en&gl=CA&_=${Date.now()}`,
            ...
            for (const url of sources) {
                try {
                    ...
                    const rate = parseGoogleCadUsd(payload);
                    if (!Number.isFinite(rate) || rate < 1 || rate > 2) throw new Error('Google quote is outside the expected range');
                    return {
                        rate,
            ...
            throw lastError || new Error('Google Finance unavailable');
        }

        async function refreshGoogleRate(forceApply = false) {
            ...
            if (status) { status.classList.remove('is-live'); status.innerText = 'Refreshing live spot for calculation…'; }
            if (refreshBtn) refreshBtn.classList.add('loading');
            if (note) note.innerText = 'Fetching Google Finance…';
            try {
                const { rate: googleSpotRate, lastUpdated, source } = await getUsdCadRate();
                ...
                // Only the Settings card displays the raw Google Finance spot rate.
                if (spotValue) {
                    spotValue.innerText = formattedSpot;
                    spotValue.style.color = 'var(--brass-deep)';
                }
                if (note) note.innerText = `${source} · fetched ${lastUpdated} · indication only`;

                // Calculator and sidebar use live USD/CAD spot plus three Canadian cents.
                document.getElementById('sidebar-rate').innerText = `1 USD = ${formattedCalculation} CAD`;
                if (status) {
                    status.classList.add('is-live');
                    status.innerText = `● Live calculation rate · spot + 0.0300 · refreshed ${lastUpdated}`;
                }

                const rateInput = document.getElementById('rate-input');
                if (forceApply || !rateInput.value) {
                ...
                console.error('Google Finance rate unavailable:', error);
                document.getElementById('sidebar-rate').innerText = 'Google rate unavailable';
                if (status) { status.classList.remove('is-live'); status.innerText = 'Live spot unavailable · enter the calculation rate manually'; }
                if (spotValue) spotValue.style.color = 'var(--rust)';
                if (note) note.innerText = 'Google Finance could not be reached. Enter the calculation rate manually.';
                return null;
            } finally {
        ...
        // Load Google Finance spot for indication and apply spot + CAD 0.0300 to calculations.
        refreshGoogleRate(false).catch(() => {});
...
// Cosmetic: keep the ticket route strip in sync with the selected corridor.
        function updateTicketDestination() {
            ...
            syncCountrySelectUI();
            const syriaCurrencyContainer = document.getElementById('syria-currency-container');
            if (country === 'Syria' && syriaCurrencyContainer.style.display !== 'none') {
                const currency = document.getElementById('syria-currency-input').value;
                if (currency === 'SYP') {
        ...
        ['country-input', 'syria-currency-input', 'syria-city-input', 'amount-input', 'rate-input'].forEach(id => {
